...
def parse_tfevent_log(root_dir):
    ...
    try:
        print("train rewards max at ======== ", all_train_returns.index(max(all_train_returns)))
    except:
...
if __name__ == "__main__":
    ...
    eval_rew = parse_tfevent_log(log_path)
    until = 4
    grad_updates = [0, 1, 2, 3, 4,5]
    # plt.plot(grad_updates, eval_rew, label='CAVIA')
    plt.plot(grad_updates[:until], eval_rew[:until], marker="o", label='CAVIA')
